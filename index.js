...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton() || interaction.isStringSelectMenu()) {
    ...
    switch (interaction.customId) {
      case 'pause':
        ...
        await interaction.reply(`${player.paused ? '⏸️ Paused' : '▶️ Resumed'}`);
        break;
      case 'skip':
        player.stop();
  ...
  if (commandName === 'play') {
    ...
    switch (res.loadType) {
      ...
      case 'NO_MATCHES':
        ...
        break;
      case 'LOAD_FAILED':
        await interaction.reply({ content: 'Failed to load track!', ephemeral: true });
        break;
    }
  }
  ...
  if (commandName === 'stop') {
    ...
    if (player) {
      player.destroy();
      await interaction.reply('⏹️ Stopped the music and left');
    } else {
